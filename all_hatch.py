...
# 根据Java服务器代码调整的格式定义
HEADER_FORMAT = "<4s HH III d I"  # 魔数(4s) + 版本(H) + 头部长度(H) + 帧ID(I) + 点数量(I) + 时间戳(I)
...
async def parse_point_cloud_data(data: bytes,HEADER_SIZE,POINT_SIZE,HEADER_FORMAT,POINT_FORMAT):
    ...
    try:
        ...
        (
                magic, version, header_len, frame_id, num_points, time_stamp,current_machine_position,hatch_num
            ) = header_data
        ...
        if len(remaining_points) > 3:
            ...
            for i, cluster in enumerate(valid_clusters):
                ...
                if len(cluster_points) >= 100:
                    ...
                    x_center = (x_avg_min+x_avg_max)/2
...
async def main():
    ...
    SERVER_PORT = 27052
    SERVER_PATH = "/leishen-point-cloud-3"
    ...
    try:
        while True:
            try:
                if not ws_manager.is_connected:
                    print(f"WebSocket断开，重连中...")
                    await ws_manager.connect()
                    
                original_points, header_info = await get_point_cloud_from_websocket_persistent(
                    ws_manager, HEADER_SIZE, POINT_SIZE, HEADER_FORMAT, POINT_FORMAT
                )
                ...
                hatch_num=header_info.get('hatch_num',0)
                result = await process_point_cloud(original_points, is_visualize)
                
                if result:
                    x_center_list=[item[0] for item in result["cluster_stats"]] #遍历result["cluster_stats"]这个二维数组，取每个元素的第一个元素，即x_center
                    x_range_list=[item[3] for item in result["cluster_stats"]] #遍历result["cluster_stats"]这个二维数组，取每个元素的第四个元素，即x_range
                    x_range=max(x_range_list)
                    #从小到大排列
                    x_center_list.sort()
                    
                    print(f"x_center_list: {x_center_list}")
                    #相邻两个元素的差值的最大值为center_gap
                    center_gap=max([x_center_list[i+1]-x_center_list[i] for i in range(len(x_center_list)-1)])
                    print(f"center_gap: {center_gap}")

                    #将x_center_list取反
                    x_center_list=[-item for item in x_center_list]
                    print(f"x_center_list取反: {x_center_list}")

                    x_center_list=x_center_list[:2]

                    #往x_center_list添加元素hatch_num-2个元素
                    x_center_list.extend([x_center_list[-1]-center_gap*(i+1) for i in range(hatch_num-2)])
                    print(f"x_center_list: {x_center_list}")

                    #加上current_machine_position
                    x_center_list=[item+current_machine_position+2.7 for item in x_center_list]
                    print(f"加上current_machine_position: {x_center_list}")

                    #封装成一个二维数组，每个元素是一个数组，数组的第一个元素是x_center，第二个元素是x_range
                    x_center_range_list=[[item,x_range] for item in x_center_list]

                    to_java_data={
                    "type":1,
                    "filtered_result":x_center_range_list
                    }
                ...
                if not isinstance(e, (websockets.exceptions.WebSocketException, 
                websockets.exceptions.ConnectionClosed,
                ConnectionError, OSError)):
                    logger.warning(f"出现问题", exc_info=True)
                    to_java_data={
                    "type":2,
                    }
                    json_message = json.dumps(to_java_data, ensure_ascii=False)
